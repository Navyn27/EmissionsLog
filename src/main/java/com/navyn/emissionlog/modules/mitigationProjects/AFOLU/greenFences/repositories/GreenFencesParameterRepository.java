package com.navyn.emissionlog.modules.mitigationProjects.AFOLU.greenFences.repositories;

import com.navyn.emissionlog.modules.mitigationProjects.AFOLU.greenFences.models.GreenFencesParameters;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.Optional;
import java.util.UUID;

@Repository
public interface GreenFencesParameterRepository extends JpaRepository<GreenFencesParameters, UUID> {

    /**
     * Finds the latest active GreenFencesParameter ordered by creation date (most
     * recent first)
     * 
     * @return Optional containing the latest active parameter, or empty if none
     *         found
     */
    Optional<GreenFencesParameters> findFirstByIsActiveTrueOrderByCreatedAtDesc();
}
